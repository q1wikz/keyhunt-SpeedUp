			...
			if(bsgs_found[k] == 0)	{
				...
				while( j < cycles && bsgs_found[k]== 0 )	{
					...
					*/
					...
					// Precompute -startP.y (invariant across loop)
					neg_sy.Set(&startP.y);
					neg_sy.ModNeg();
					
					for(i = 0; i<hLength; i++) {

						dy.ModSub(&GSn[i].y,&startP.y);
...
pn.y.ModAdd(&GSn[i].y);
					...
					{
					// Pipeline depth=2: prefetch i+1 and i+2 while checking i
					char xpoint_buf[3][32];
					uint64_t pipe_a[3], pipe_b[3];
					int pipe_idx;

					// Prime pipeline: hash+prefetch elements 0 and 1
					pts[0].x.Get32Bytes((unsigned char*)xpoint_buf[0]);
					bloom_hash_and_prefetch(&bloom_bP[((unsigned char)xpoint_buf[0][0])],xpoint_buf[0],32,&pipe_a[0],&pipe_b[0]);
					if(CPU_GRP_SIZE > 1) {
						pts[1].x.Get32Bytes((unsigned char*)xpoint_buf[1]);
						bloom_hash_and_prefetch(&bloom_bP[((unsigned char)xpoint_buf[1][0])],xpoint_buf[1],32,&pipe_a[1],&pipe_b[1]);
					}

					for(int i = 0; i<CPU_GRP_SIZE && bsgs_found[k]== 0; i++) {
						pipe_idx = i % 3;
						struct bloom *cur_bloom = &bloom_bP[((unsigned char)xpoint_buf[pipe_idx][0])];

						// Hash+prefetch element i+2 while memory for i is being accessed
						if(i + 2 < CPU_GRP_SIZE) {
							int next2_idx = (i + 2) % 3;
							pts[i+2].x.Get32Bytes((unsigned char*)xpoint_buf[next2_idx]);
							bloom_hash_and_prefetch(&bloom_bP[((unsigned char)xpoint_buf[next2_idx][0])],xpoint_buf[next2_idx],32,&pipe_a[next2_idx],&pipe_b[next2_idx]);
						}

						r = bloom_check_prehashed(cur_bloom, pipe_a[pipe_idx], pipe_b[pipe_idx]);
