## Оптимизации KeyHunt BSGS по порядку
	Оптимизация	          Было → Стало	        Прирост
1.	Pipeline bloom prefetch depth=2 — hash+prefetch i+2 пока проверяем i, скрывает cache miss latency на 8 GB bloom	81 → 96	+18.5%
2.	Inner loop neg_sy + x-only — precompute -startP.y, убраны полные Point copy (pp=startP/pn=startP), вычисляется только .x	96 → 108	+12.5%
3.	ASM ModMulK1 (mulx+adcx+adox) — ассемблерная Montgomery multiplication вместо compiler-generated	108 → 116	+7.4%
4.	ASM ModSquareK1 — специализированный squaring, inner loop ускорился с 24.4% → 21.8%	~116	~0% отдельно (bloom доминирует)
5.	Bloom filter bit indexing fix — & mask вместо % bits + power-of-2 rounding размера bloom	Баг-фикс	Корректность + скорость
6.	Bloom error rate 1e-6 → 1e-3 — bloom в 2x компактнее → освободилась RAM для удвоения k	4 GB → 8 GB bloom, k 512→1024	~2x покрытие диапазона

## Суммарный эффект
Чистая скорость (при равном k=512): 80 → ~128 Pkeys/s (+60%)
С удвоением k (512→1024): 80 → 255 Pkeys/s (+3.2x), при этом каждый шаг покрывает вдвое больший диапазон
Память: bloom error=0.001 позволяет k=1024 в тот же объём RAM, который раньше занимал k=512 при error=0.000001

-------------------------------------------------------------------
### output:
`./keyhunt -m bsgs -f tests/135.txt -b 135 -R -k 1024 -q -t 12 -s 10 -S`
```
...
[+] N = 0x100000000000
[+] Bloom filter for 4294967296 elements : 8192.00 MB
[+] Bloom filter for 134217728 elements : 512.00 MB
[+] Bloom filter for 4194304 elements : 16.00 MB
[+] Allocating 64.00 MB for 4194304 bP Points
[+] processing 4294967296/4294967296 bP points : 100%
[+] Making checkums .. ... done
[+] Sorting 4194304 elements... Done!
[+] Writing bloom filter to file keyhunt_bsgs_4_4294967296.blm .... Done!
[+] Writing bloom filter to file keyhunt_bsgs_6_134217728.blm .... Done!
[+] Writing bP Table to file keyhunt_bsgs_2_4194304.tbl .. Done!
[+] Writing bloom filter to file keyhunt_bsgs_7_4194304.blm .... Done!
[+] Total 25515671454448877568 keys in 100 seconds: ~255 Pkeys/s (255156714544488775 keys/s)
```
-------------------------------------------------------------------
