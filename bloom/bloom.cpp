...
static int bloom_check_add(struct bloom * bloom, const void * buffer, int len, int add)
{
  ...
  uint64_t mask = bloom->bits_mask;
  uint8_t *bf = bloom->bf;
  uint64_t x;
  uint8_t i;
  for (i = 0; i < bloom->hashes; i++) {
    x = (a + b*i) & mask;
    if (test_bit_set_bit(bf, x, add)) {
      hits++;
    } else if (!add) {
      return 0;
    }
...
int bloom_init2(struct bloom * bloom, uint64_t entries, long double error)
{
  ...
  // Round bits up to next power of 2 for fast AND operation
  uint64_t bits_pow2 = next_power_of_2(bloom->bits);
  bloom->bits_mask = bits_pow2 - 1;
  bloom->bits = bits_pow2;
...
int bloom_check(struct bloom * bloom, const void * buffer, int len)
{
  ...
  uint8_t i;
  for (i = 0; i < bloom->hashes; i++) {
    uint64_t x = (a + b*i) & mask;
    if (__builtin_expect(!test_bit(bf, x), 1)) {
      return 0;
    }
  }
  return 1;
...
void bloom_hash_and_prefetch(struct bloom * bloom, const void * buffer, int len, uint64_t *out_a, uint64_t *out_b)
{
  uint64_t a = XXH64(buffer, len, 0x59f2815b16f81798);
  uint64_t b = XXH64(buffer, len, a);
  *out_a = a;
  *out_b = b;
  uint64_t x = a & bloom->bits_mask;
  __builtin_prefetch(&bloom->bf[x >> 3], 0, 0);
}

int bloom_check_prehashed(struct bloom * bloom, uint64_t a, uint64_t b)
{
  uint64_t mask = bloom->bits_mask;
  uint8_t *bf = bloom->bf;
  uint8_t i;
